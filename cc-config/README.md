...
Non-secret provider *shape* lives in `~/.claude/cc-config.json`.  API
...
### `cc-config activate <provider-name/model-name>`
...
### `cc-config remove <provider-name>`
...
| Path | Purpose |
|------|---------|
| `~/.claude/cc-config.json` | Provider/model shapes (no secrets) |
| `~/.claude/settings.json` | Generated by `activate` |
| credstore (`DEEPSEEK_API_KEY`, …) | The actual API key values |

`CC_CONFIG_FILE` overrides the config path; the settings path can be
...
- Secrets are immutable Python `str` — cc-config follows credstore's

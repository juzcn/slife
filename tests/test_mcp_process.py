...
class TestMCPWrapperProcessInit:
    ...
    def test_default_args_use_server_module(self):
        wp = MCPWrapperProcess()
        assert wp._args == ["-m", "slife.plugins.mcp_gateway.server"]
...
class TestMCPWrapperProcessStart:
    ...
    async def test_already_running_logs_warning(self):
        ...
        with patch("slife.plugins.mcp_gateway.process.logger") as mock_logger:
            await wp.start()
        ...
        with patch("asyncio.create_subprocess_exec") as mock_create:
            ...
            with patch("slife.plugins.mcp_gateway.process.logger"):
                with patch.object(wp, "_read_port_signal", AsyncMock()), \
                    ...
                    with patch("slife.plugins.mcp_gateway.process.get_session_id",
                               return_value="test-sid-1234"):
                        with patch("slife.plugins.mcp_gateway.process.asyncio.create_task"):
                            await wp.start()
...
class TestMCPWrapperProcessCreateClient:
    ...
    async def test_creates_client_successfully(self):
        ...
        with patch("slife.plugins.mcp_gateway.client.MCPClient") as MockClient:
...
class TestMCPWrapperProcessStop:
    ...
    async def test_stop_not_running_noop(self):
        wp = MCPWrapperProcess()
        with patch("slife.plugins.mcp_gateway.process.terminate_process") as mock_term:
            await wp.stop()

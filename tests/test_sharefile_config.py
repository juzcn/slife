"""sharefile.yaml — path resolution, provider selection, and degradation.
...
class TestResolveConfigPath:
    ...
    def test_default_follows_the_data_dir(self, tmp_path, monkeypatch):
        ...
        assert cfgmod.resolve_config_path() == tmp_path / "sharefile.yaml"


class TestLoadConfig:
    def test_missing_file_falls_back_to_ngrok(self, tmp_path):
        cfg = cfgmod.load_sharefile_config(tmp_path / "absent.yaml")
    ...
    def test_unknown_active_provider_falls_back(self, tmp_path):
        cfg = cfgmod.load_sharefile_config(
            _write(tmp_path, "active_provider: made-up\nproviders: {}\n")
        )
        assert cfg.active_provider == "ngrok"

    def test_broken_config_does_not_raise(self, tmp_path):
        cfg = cfgmod.load_sharefile_config(_write(tmp_path, "providers: [unclosed"))
        assert cfg.active_provider == "ngrok"
        assert cfg.providers == {}
    ...
    def test_provider_resolves_env_references(self, tmp_path, monkeypatch):
        ...
        providers:
          localhost.run:
            ssh: ${SHAREFILE_TEST_SSH}
            remote_port: ${UNSET_PORT:-8022}
        """))
    ...
    def test_provider_with_unresolved_env_is_dropped_alone(self, tmp_path):
        ...
        providers:
          localhost.run:
            ssh: ${SHAREFILE_DEFINITELY_UNSET_VAR}
          ngrok: {}
        """))
...
class TestShippedConfig:
    """The git-tracked sharefile.yaml is what every install seeds."""

    def test_parses_and_names_a_known_provider(self):
        root = Path(__file__).resolve().parents[1]
        cfg = cfgmod.load_sharefile_config(root / "sharefile.yaml")
        ...
        for name in KNOWN_PROVIDERS:
            assert name in cfg.providers, f"{name} missing from sharefile.yaml"

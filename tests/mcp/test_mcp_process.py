...
class TestMCPWrapperProcessStart:
    ...
    async def test_already_running_logs_warning(self):
        ...
        with patch("slife.plugins.mcp.process.logger") as mock_logger:
            await wp.start()
    ...
    async def test_start_creates_subprocess(self):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_proc):
            with patch("slife.plugins.mcp.process.logger"):
                with patch.object(wp, "_read_port_signal", AsyncMock()):
                    with patch.object(wp, "_log_stderr", MagicMock()), \
                         patch("slife.plugins.mcp.process.asyncio.create_task", lambda c: None):
                        await wp.start()
        ...
        with patch("asyncio.create_subprocess_exec") as mock_create:
            ...
            with patch("slife.plugins.mcp.process.logger"):
                with patch.object(wp, "_read_port_signal", AsyncMock()), \
                    ...
                    with patch("slife.plugins.mcp.process.get_session_id",
                               return_value="test-sid-1234"):
                        with patch("slife.plugins.mcp.process.asyncio.create_task"):
                            await wp.start()
...
class TestMCPWrapperProcessCreateClient:
    ...
    async def test_creates_client_successfully(self):
        ...
        with patch("slife.plugins.mcp.client.MCPClient") as MockClient:
...
class TestMCPWrapperProcessStop:
    ...
    async def test_stop_not_running_noop(self):
        wp = MCPWrapperProcess()
        with patch("slife.plugins.mcp.process.terminate_process") as mock_term:
            await wp.stop()
...
class TestMCPWrapperProcessLogStderr:
    ...
    async def test_log_stderr_relays_non_banner_lines(self):
        ...
        with patch("slife.plugins.mcp.logging.read_stderr_lines") as mock_read:
            async def _gen():

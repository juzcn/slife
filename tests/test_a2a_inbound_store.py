...
class TestRestartOrphansPending:
    ...
    def test_dropping_an_unknown_id_is_a_noop(self, tmp_path):
        store = InboundStore(tmp_path / "state.yaml")
...
class TestRobustness:
    ...
    def test_a_missing_file_is_not_an_error(self, tmp_path):
        assert InboundStore(tmp_path / "absent.yaml").stale() == []
    ...
    def test_state_is_written_on_disk(self, tmp_path):
        ...
        # YAML, not JSON — the file a human opens reads as a document.
        assert path.read_text(encoding="utf-8").startswith("pending:")

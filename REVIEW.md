...
**Current state:** full suite **1761 passed** (re-confirmed after this pass's fixes). This pass fixed: memdb Chinese→English, `_sys_trim` targeting the wrong conversation, plugin-tool double-prefix naming, `rest_api_set_enabled(false)` ignored at startup, `cli_set_enabled` not persisting, the harness `__` filter inconsistency, memfiles health pointer not refreshed on watchdog restart, the worst unredacted log call sites, **C1** (A2A `transport:"http"` no longer crashes startup — non-`"mqtt"` transports disable A2A with a warning; the dead `HttpStreamableTransport` stub was removed), **C9** (streamable-http `_request_http` now reads SSE-streamed responses), **C2** (external MCP servers now get a background health-check — a died or hung server is marked `DISCONNECTED` and reconnected with backoff instead of staying `CONNECTED`), **W1/W2** (Anthropic and OpenAI-Responses backends now emit their own wire formats — Anthropic coalesces a multi-tool-result batch into one `user` message so messages strictly alternate; Responses emits `function_call`/`function_call_output` items instead of the Chat-Completions `role:"tool"`/`tool_calls` shape), **C4/C5** (subagent `_read_stdout` no longer strands pending futures on a malformed message; A2A `CancelTask` and subagent `worker/cancel` now truly preempt the running agent loop via the same Esc mechanism — and the mesh `a2a_cancel_task` never mislabels a completed result as cancelled), **C6** (WeChat dedup no longer drops real messages — the key includes the text and non-text items no longer burn it), **C7** (the approval dialog's Esc-deny is no longer stolen by the App's priority `escape → cancel` — the modal declares its own priority deny binding), and **C8 + M2–M7** (all remaining Correctness items: reserved built-in MCP names; watchdog no longer stalls on a wedged child; mcp lifespan shuts down the connection pool; stdio teardown kills the process tree; memdb LIKE `ESCAPE '\'`, fts5 count honors `since`/`until`, search `limit` clamped; background reindex bounded against a failing embedder).
...
None open — every Correctness item from prior passes (C1, C2, C4–C8, W1/W2, M2–M7) is fixed; see §1.3.
...
| **C6** | WeChat dedup key (`from_user_id + context_token`) could **drop real messages** — `context_token` is per-conversation, so a second message in a conversation collided with the first, and non-text items burned the key before the empty-text check | `_msg_key` now includes the text (dedup only true re-deliveries) and the empty-text check runs **before** the key is recorded (non-text items no longer burn it); 5 tests added (`plugins/wechat/server.py`) |
| **C7** | Approval dialog "Deny (Esc)" intercepted by the App's **priority** `escape → cancel` binding — the modal's deny never fired, the loop cancelled instead, and the awaited approval future stayed unresolved (modal stuck) | `ApprovalDialog` now declares its own **priority** `enter → approve` / `escape → deny` bindings (fire before the App's in the widget tree), backed by `action_approve`/`action_deny`; removed the never-reached `on_key`; 5 tests added (`ui/approval_dialog.py`) |
| **C8** | MCP dispatch keyed on raw server name; `mcp`/`memdb`/`wechat`/`memfiles`/`a2a` not reserved — a colliding external server name misroutes in the harness namespace | `mcp_set` rejects any name in `_RESERVED_SERVER_NAMES` (the five built-in plugin names) — covers runtime `mcp_set` and the config-load auto-connect (both go through `mcp_set`); 1 test added (`plugins/mcp/server.py`) |
| **M2** | Watchdog restart can stall on a live-but-unconnected child — `self.process` was assigned before `create_client`, so a failed connect left the lifecycle pointing at a wedged child and `subprocess.wait()` blocked forever (backoff defeated) | `spawn`/`_connect_mcp_wrapper`/`_spawn_plugin_generic` now reset `process`/`client`/`port` and stop the child on any failure before re-raising, so the watchdog backs off and retries; 1 test added (`agent/plugins.py`, `agent/service.py`) |
| **M3** | mcp plugin lifespan never shut down `ConnectionPool` — HTTP/SSE external servers leaked on exit | mcp plugin now declares a `lifespan` that `await _pool.shutdown()`s on the server's event loop (a2a plugin already disconnects its client via its own lifespan); 1 test added (`plugins/mcp/server.py`) |
| **M4** | External MCP stdio teardown killed only the direct child — npx/uvx grandchildren survived on Windows | `_cleanup_resources` now kills the whole tree (`_kill_process_tree`: `taskkill /T` / `killpg`), which also runs when the direct child already exited (grandchildren may be orphaned); `_kill_process_tree` no longer early-returns on `returncode is not None`; 1 test added (`plugins/mcp/connection.py`, `tools/exec.py`) |
| **M5** | memdb `search_grep` LIKE-escaping was a no-op (no `ESCAPE '\'` clause) — `%`/`_` patterns matched nothing | `search_grep` now doubles backslashes, escapes `%`/`_`, and the LIKE clauses carry `ESCAPE '\'`; 1 test added (`plugins/memdb/store.py`) |
| **M6** | memdb `count_turns` fts5 mode ignored `since`/`until`; search `limit` unclamped (negative → unlimited) | fts5 count now joins `diary` and applies the time clauses; all LLM-facing search methods clamp `limit` to `[1, 200]` via `_clamp_limit`; 2 tests added (`plugins/memdb/store.py`) |
| **M7** | memdb background reindex could spin forever on a persistently failing embedder | `_background_reindex` bounds consecutive no-progress batches (`_MAX_REINDEX_NO_PROGRESS = 20`) and gives up with a warning instead of spinning; 2 tests added (`plugins/memdb/server.py`) |
...
Five built-in plugins (`mcp`, `memdb`, `wechat`, `memfiles`, `a2a` — the mqtt plugin became a2a), all Streamable HTTP, all `create_plugin_server` + `run_plugin_server`. **Fixed this pass:** double-prefix naming (F-prefix), memdb Chinese (F-lang), memfiles health pointer (F-memfiles), and plugin-lifespan pool/client shutdown (M3 — mcp pool + a2a client). **Remaining:** watchdog gap for third-party plugins (P4) and `plugins/__init__.py` stale docstring.
...
**Current:** **1761 pass in ~26s** (regression tests added for H1–H8, M1 in prior passes; this pass added 11 health-check/reconnect tests for C2, 5 backend wire-format tests for W1/W2, 5 subagent `_read_stdout`/`_dispatch_message` tests for C4, 4 mesh `cancel_task` tests + 15 true-cancellation tests for C5, 5 WeChat dedup tests for C6, 5 approval-dialog binding tests for C7, and 9 tests for C8 + M2–M7, plus test-assertion updates for the memdb translation).

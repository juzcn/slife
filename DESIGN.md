...
Every plugin-backed check probes the plugin's internal `__check` tool, which reports only raw technical state (facts and measurements, like a physical-examination report) and **never triggers a connect**. The harness interprets those facts into health levels and remediation hints — a plugin `__check` has no levels of its own.  A report also states its own **scope** when a class of facts was not probed in the asking process: the host facts (config path, active model, external tool versions) are recorded by the interactive startup, so a worker's report names them as absent instead of letting its smaller component count read as a smaller system. The watchdog only monitors processes — it does not introspect application state.

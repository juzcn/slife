...
`_auto_invoke` — each `result.startswith("Error")`), stores it on the tool
...
**Trigger → execution.** The loop injects a `[Schedule <name>]` message into the unified inbox under the **system channel** (source `SYSTEM`, `channel=system`, silent handler like heartbeat — excluded from "remote" handling in `inbox.py`); the `[Schedule <name>]` text is the *schedule marker* riding that channel's content. The run itself is recorded when the agent dispatches, not at fire time, and a short in-memory pending-fire guard keeps the 30 s poll from re-firing while the agent is mid-turn. The agent handles the trigger by delegating, not doing: it calls `run_schedule_now` — the single dispatch tool, also used to backfill missed runs — which records a `scheduled_runs` row (`status='pending'` — success is unconfirmed until a report lands), spawns/reuses the subagent named after the task, and sends it a deterministic task text via `subagent_send_task_async` (mode=auto) that instructs the worker to call `report_save` when done and to notify the user. Completion rides the existing subagent auto-push back to the main agent, which reports the task as finished (reworded to hide the subagent). Idle schedule workers are recycled by the loop once their task has no run still in flight.

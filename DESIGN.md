...
**Run outcomes are optimistic-pending.** A fire is recorded `pending`; a report arriving flips it to `ran` (the *only* success writeback, at the store layer alongside the `report_id` backfill). Everything else is a failure by default: the trigger turn's reply hook may best-effort mark the run failed with a reason (interrupted / the turn's error), and at startup the loop sweeps any surviving `pending` (and legacy `ran`-without-report rows) to `failed` — a previous process lifetime can never complete them.

**Missed & failed runs.** Both surface for user choice in one `[Schedule missed]` notice per process: *missed* (slife was down at the due time) and *failed* (dispatched but never produced a report, typically interrupted/errored/restarted). The agent tells the user and offers to backfill (`run_schedule_now` fires a task immediately, recording a run) or skip (`scheduled_run_skip` closes the missed/failed run — a *skipped* run is the user's "don't backfill", never a success marker). Scheduled tasks fire **only while slife is running** — a run due while closed is missed, never silently lost.

Tools: `scheduled_task_set` / `scheduled_task_remove` / `scheduled_task_list`, `scheduled_run_list` / `scheduled_run_skip`, `save_cron_report` / `report_list` / `report_read` (all memfiles MCP tools, visible to main agent and worker alike), plus the native `run_schedule_now` (the one operation that needs the main-process inbox, reached through the `fire_schedule_now` tool-context hook).

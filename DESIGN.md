...
- **Channel A2A markers**: the sender's wire envelope carries `_slife.kind` (`"task"` or `"message"`). Inbound messages reach the model prefixed `[A2A:{"agent_name": …, "task_id": …}]` — `task_id` present only for a task; auto-pushed async results use `[A2A-PUSH:…]` with the same shape. Both markers are machine-facing: the TUI shows `A2A(<peer>)>` and strips them.

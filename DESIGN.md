...
>
> Three documents sit beside it and are deliberately **not** duplicated here:
...
> philosophy, the trade-offs, the next refactor; and **[INVARIANTS.md](INVARIANTS.md)** is the
> numbered list of rules that must not be broken, each with what violating it costs. This document
> states the design; that one states the assertions, and the code cites either by section.
...
for when the current model is unavailable and the model cannot switch itself — see
...
Four interaction rules are load-bearing, and [INVARIANTS.md](INVARIANTS.md) carries the canonical
statement of each: the app's cancel binding must **not** be priority, the approval prompt's and model
picker's **must** be, a binding action must be **sync**, and tool widgets are cleared only at the
genuine turn-end event.
...
The invariants this design rests on are collected in **[INVARIANTS.md](INVARIANTS.md)**.

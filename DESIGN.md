...
**Write path is insert-only.** `save_turn` persists the row and never embeds on the save path (a slow GGUF embed of a large turn previously tripped the harness's 10s save timeout — a false alarm; the row was saved anyway). Embedding is an internal plugin concern: `__memory_save_turn` kicks `_ensure_index_complete()` right after each insert, so the turn becomes semantically searchable shortly after save once the gate re-opens. `memory_summarize` is the one in-place re-embed path: after updating summary/tags it clears the turn's old chunks and embeds the summary as a single chunk.

**Index-completeness gate.** Semantic results are served only when `count_unembedded() == 0` (`_semantic_ready`). `_ensure_index_complete()` — run on store init and at the start of every `memory_search` / `memory_check_embedding` — materialises the model if needed (an already-complete index would otherwise never trigger the lazy load and the gate would lock off forever), then starts/resumes `_background_reindex` while any turn is unembedded. The reindex embeds in small batches (5 turns, 0.5s apart) and flips the gate back ON when the index is complete; a persistently failing embedder is bounded by a no-progress limit. While the gate is OFF, hybrid degrades to FTS5-only with a hint naming the reason — partial semantic results are never served.
...
**Model / dimension change.** `memory_set_embedding` / `memory_set_enabled(True)` re-run the store setup; a vec0 width change drops and recreates the table (old vectors live in a different vector space) and triggers a reset reindex. The same path catches a model/dimension change made by a manual `slife.json5` edit before the gate re-enables.
...
- Async task results **auto-push**: a peer's result arrives on `Slife/<agent_name>/tasks/result` over MQTT → the client fires `on_task_result` → the plugin queues a completion → `_a2a_poll_loop` drains it into the conversation ("Peer X completed async task (ID: …): …"). The agent sends async and the result simply arrives — no polling or blocking (MQTT subscription is implicit, so the HTTP/SSE-style `a2a_subscribe_task` was dropped).

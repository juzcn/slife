...
1. **A new user message, marker-carrying — persistent.** The heartbeat (`[Heartbeat]`) and scheduled-task triggers (`[Schedule <name>]`, `[Schedule missed]`) are posted to the inbox as synthetic user messages. Each runs as a normal agent-loop turn and is saved to the diary like any turn. The marker lets the TUI filter the trigger (never shown) and surface only a real reply (`⚡ 自主`). On **restore**, a non-human turn's `channel` is rendered as a unified `[Kind:{json}]` identity marker (heartbeat, schedule, subagent, wechat, remote) so the model and the restored TUI both see the same identity grammar; the legacy `[Heartbeat]` / `[Schedule …]` prefix is folded into it. See *Autonomous Heartbeat* and *Scheduled Tasks*.
...
| `channel` | Source: `human`, `wechat`, `subagent`, `heartbeat`, `schedule`, or remote agent id. Restore derives the unified `[Kind:{json}]` identity marker from it; `human`/empty carries no marker (absence == human) |
...
**Turn headers on restore.** Each restored user message gets a compact `[Turn: N · start → end]` footnote (rowid + created → completed) concatenated into the message text — without it the whole restored history would read as "just happened". The LLM can also use N with `turn_read` / `turn_summarize`, and the human reads the same line in the TUI. Restored turns get it from persisted columns at restore; a just-completed live turn gets the same footnote appended by `save_to_memory` once `__memory_save_turn` returns its rowid — so the model can reference the previous turn precisely, and `turn_summarize`'s latest-rowid default stops being racy. **The footnote is runtime-only and never persisted:** the stored `user_message` / `messages` are written *before* the append, restore regenerates it from columns, so the DB carries the clean original in both paths. The current in-flight turn carries none (it is the one that IS now), and live TUI bubbles are not retro-updated — a missing footnote is itself the "current session" signal.

**Unified identity marker.** A restored **non-human** user message additionally gets a `[Kind:{json}]` marker prepended, derived from the persisted `channel` column (`slife/a2a/markers.py`): `[Subagent:{"subagent_name": …}]`, `[Heartbeat:{}]`, `[Schedule:{"name": …}]`, `[Wechat:{}]`, `[Remote:{"peer_id": …}]`. **Human turns carry no marker — absence == human.** The marker is the single carrier of turn identity for the LLM and for restore-time display/filtering (the `subagent(a2a)` mislabel became impossible once display parses the marker instead of guessing from a bare `channel` string). It is **restore-time only**: the live message stream is untouched — live turns keep only the `[Turn: N]` / `[TrimContext: N]` markers, and the legacy `[Heartbeat]` / `[Schedule <name>]` prefixes stay exactly as injected. On restore, a legacy prefix is folded into the unified marker (stripped, identity moved into the JSON payload). An unreadable image attachment raises a `ValueError` from `add_user_message` instead of being silently dropped — upstream (`@path` parsing, `attach_image`) validates files first, so a failure here is a bug signal, not a hidden marker.
...
Each kind owns its FTS5 + vec0 tables (`notes_*` / `diary_*` / `files_*`). `cabinet_search(query, kind, mode)` runs hybrid (FTS5 + vec0 KNN, RRF via the shared `merge_hybrid`) or keyword search across them; `cabinet_read(path)` re-opens a file with a path-traversal guard. Browsing by key: `note_list` / `diary_list` list entries (newest first, diary optionally by date range) and `note_read(subject)` / `diary_read(date)` return full content. The index mirrors memdb's design and **reuses its code**: the shared `SemanticManager` (document-source contract) drives the drainer over all three kinds, and `cabinet_embedding_check` reports this index's own gate — independent from memdb's `semantic_index_status`, because each plugin reindexes its own DB (one shared `memdb.embedding` config, independent availability).

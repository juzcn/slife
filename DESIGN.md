...
> Developer documentation for the slife codebase. For installation, configuration, and everyday usage, see [README.md](README.md).  Terminology is defined where it first matters in this document. This document covers the design principles, the agent loop, the tool system, the plugin contract, the MCP gateway, the memory database, the A2A mesh, the credential security model, and the full project structure. It is written for people who work on the code, and it assumes you have read the README.
...
7. [Plugin Architecture](#plugin-architecture) — contract, localhost/proxy, watchdog, built-ins, MCP gateway, subagent discovery, job system
...
│  mcp-gateway (gateway) · memdb (turns DB) · wechat         │
│  a2a (MQTT) · memfiles (files) · sharefile         │
│  media (generation) · local-embed (embeddings)                 │
├──────────────────────────────────────────────────────────────────────┤
...
- **Background execution**: per-call `_async: true` schedules the tool as a background task and returns a task id immediately; poll with `check_async`, cancel with `cancel_async`. The async runner **sanitizes at storage time** (secrets are scrubbed the moment the task finishes, not when polled) and a **failed** async task surfaces with the `Error:` prefix — the same `is_error` contract as a synchronous call — instead of being wrapped in a success banner
...
| `__check` / `__mcp_call_tool` / `__mcp_get_tool` | mcp plugin | Internal — invisible |
...
Three tool tiers exist, indistinguishable to the LLM: **native** tools (own names), **built-in plugin** tools (bare names), and **external MCP server** tools (`{server}__{tool}`).  The naming rule is fixed; the developer-facing mechanism is described below under [Tool Categories](#tool-categories).
...
Every tool returns a single string (`async execute(**kwargs) -> str`). The
failure contract is one rule, one token: **a failed call returns a string
starting with `Error:`**. The harness derives the persisted `is_error` flag
from exactly that prefix at both dispatch sites (`_execute_tools` and
`_auto_invoke`), judged **before** the args-truncation marker (`⚠ Provider
truncated this tool call's arguments …`) is prepended — so a failed call whose
result began with `Error:` still reads as an error even when the marker leads
the text. The flag is stored on the tool
message, and session restore reads the stored flag rather than re-deriving it
(`tool_result_is_error`). Every tool conforms to the prefix: native tools
return `Error:`-prefixed strings directly, and wrappers over a plugin's JSON
error envelope (`{"status": "error", "error": …}`) translate it to an
`Error:`-prefixed string before returning (e.g. `mcp_tool_load` with a bad
`full_name`) — there is deliberately no second failure token (`[FAIL]`, raw
JSON) and the harness recognises none.
...
- **Bare `timeout` alias** — LLMs routinely append a schema-less `timeout` arg to tools that define no such parameter (a2a sends, subagent sends, plugin proxies); schema validation previously rejected the **whole** call with "Unexpected keyword argument". The loop pops a bare `timeout` off a tool with no native one and enforces it as the per-call bound, exactly like `_timeout` — a tool WITH a native timeout keeps its schema-valid argument untouched.
...
memfiles, sharefile, a2a, media, job-coding, and the MCP gateway
**mcp-gateway**.  Every plugin is declared by one row in the central plugin
spec (`slife/plugins/spec.py`) and driven by the same uniform lifecycle
(spawn → MCP-handshake readiness → watchdog → health); the authoritative
contract is [PLUGIN_CONTRACT.md](PLUGIN_CONTRACT.md).  There is no
`plugins.external` mechanism — third-party capability enters only as a
standard MCP server in `mcp-plugin.json5`, connected by the internal
**mcp-gateway** plugin. `local-embed` is **not** a plugin: it is a standalone daemon (started
...
gateway's own client (`slife/plugins/mcp_gateway/connection.py`), which keeps
...
| On crash | Unregisters the plugin's exact registered bare-name tools, disconnects the dead client, then re-runs the uniform start (spawn + glue) |
...
Auto-discovered third-party plugins get the same watchdog: `PluginRegistry.ensure` gives any undeclared plugin a generic `PluginLifecycle`, so a crash restarts it with the same backoff as the spec-declared built-ins.
...
| **wechat** | Streamable HTTP | Bidirectional WeChat messaging via iLink ClawBot. Long-poll loop for incoming messages (a failed poll — signalled by `client.last_error`, which `poll_updates()` sets on a swallowed network error — backs off the next poll exponentially to 30 s and resets on the next clean poll), typing indicators. Incoming messages enter the inbox as WeChat-channel turns prefixed `[Wechat:{...}]` (model-facing; the JSON carries `peer_wechat_id` / `context_token` for `wechat_send_message`, and the TUI strips the marker from display since the `Wechat>` bubble prefix already shows the channel); the model replies itself by calling the LLM-visible `wechat_send_message` — no harness auto-dispatch — addressing the peer by `peer_wechat_id` (from the marker or `wechat_check_status.last_contact`). |
...
DESIGNER_NOTES §6.7 — *"大模型越聪明，越需要一个 Job System"* · Coding Agent. A **Job** is a plain public function in `<data_dir>/jobs/*.py` (dev: `<project>/jobs/` — the repo's committed `jobs/` holds the bundled `translate`/`summarize`/`total_tokens` samples; prod: `~/.slife/jobs/`, seeded from those by the installers). Following standard MCP tool norms, the function's `__name__`, docstring, and typed signature become the job tool's name, description, and parameters schema. **The files are the source of truth — there is no job-config file**: a restart (or the generic watchdog) re-scans the directory and re-registers the tools. Creating/editing a job is *coding*: the `job-coding` skill (a standard repo skill in `skills/`) teaches the conventions and is the authoring guide the agent loads before `job-write`.

Execution is deterministic: the tool calls the job function with exactly its declared arguments; the only LLM access is an explicit `llm.chat(system=…, user=…, model=…)` one-shot on the job model — `job_coding_model`, a **top-level** `"provider/model"` ref in slife.json5 that reuses `models.providers` and is independent of `active_model`. It should name a *different* (usually smaller/faster) model: a nested one-shot job call on another provider/model neither churns the agent loop's prompt-cache prefix on the active model nor competes for its quota. Jobs that call `llm` are `async def`; pure-computation jobs stay plain `def` (the runner runs sync jobs on a **daemon thread** via `slife.threads.run_daemon` — never `asyncio.to_thread`, whose default-executor workers are joined at exit and would wedge plugin shutdown on a hung blocking job; the runner captures `contextvars.copy_context()` so a sync job's `llm` client stays visible). No system prompt, no conversation history, no agent loop ever reaches a job's model — messages are built solely from the job's arguments, a structural guarantee.

**DB-locating jobs.** `jobs/total_tokens.py` reads the memdb DB and resolves it **agent-scoped**: `$SLIFE_AGENT_NAME.db` (falling back to `slife.db`) in the data dir first — verified by a `diary`-table probe (`_looks_like_memdb`) — then any memdb-shaped `*.db` near the data dir / cwd, newest first. In a multi-agent install each agent counts its own usage.

A job that needs an **external capability** — a conversion the plugin process lacks, a GitHub/playwright/search call, or a tool the main agent never loaded — reaches it through the `mcp` handle: `from slife.plugins.job_coding import mcp`, then `await mcp.call(server, tool, args)` for ONE bare tool call, `server` being a name from `mcp-plugin.json5` and `tool` the name without the `{server}__` prefix. The handle is a lazy proxy to the **mcp-gateway** plugin: the first call connects a `client.MCPClient` to the gateway's Streamable HTTP endpoint and forwards to its internal `__mcp_call_tool` — the *same call shape* the host's `{server}__{tool}` proxies use (`tool_adapter._MCP_CALL_TOOL`). Nothing external is ever spawned a second time: jobs ride the gateway's persistent connections, so a job can use **any tool on any connected server, loaded or not**. Port discovery is lazy and layered because the gateway's port is dynamic and plugins start in parallel: the host pushes the port on every gateway connect/reconnect (`_wire_mcp_glue` → `_push_gateway_port_to_jobs` → the internal `__set_mcp_gateway_port` tool), the spawn-time `SLIFE_MCP_GATEWAY_PORT` env is the fallback, and a port change drops the cached client so the next call reconnects. `mcp.call` returns the tool's text or a deterministic `Error: ...` string (gateway unreachable, server disconnected/disabled, unknown tool) — it never raises.

**Management tools** (bare names, `ProxyRoute.DIRECT`): `job-list`, `job-write` (writes `<name>.py` — creates or replaces; (re)registers the tool immediately, persists across restart, and rolls back to the previous code on a broken write), `job-remove` (delete file + unregister), `job-run` (generic executor by name — also the execution path before the harness resync picks up a brand-new per-job tool). After any tool-set mutation the plugin pushes the standard MCP `notifications/tools/list_changed`; AgentService's generic `_rescan_plugin_tools(name)` re-lists the plugin and diffs the registry — the same dynamic-tool mechanism the mcp wrapper uses, so per-job tools appear/disappear live on the agent. Watched by the uniform watchdog (`_arm_watchdog`) and covered by `system_health` via `check_job_coding` (probes the plugin's internal `__check` through `ToolContext.job_coding_client`).

### mcp-gateway — External MCP Gateway

Three wire transports, one connection class (`MCPServerConnection`) built on the **official MCP SDK `ClientSession`** — the same SDK mechanism `client.MCPClient` uses to reach slife's own plugin children (E2). The class supplies the lifecycle the SDK does not: OAuth device flow, transport selection, health monitor (ping + reconnect with backoff), stdio stderr relay, per-server connect locking, and the `needs_user_auth` pause. The raw-JSON-RPC client it replaced (a ~1200-line hand-rolled stack duplicating `ClientSession`'s protocol layer) was deleted.

| Transport | Mechanism | Use |
|-----------|-----------|-----|
| **stdio** | Spawn subprocess via the SDK's `stdio_client` (JSON-RPC over pipes) | Local MCP servers (npx/uvx/bunx) |
...
For `url`-configured servers the gateway tries the SDK `sse_client` first: a non-event-stream reply makes its `enter` fail and the connection falls through to `streamable_http_client` (building a lazily-shared httpx2 `AsyncClient` with the resolved headers). A Streamable response may be a single JSON body or an SSE stream — the SDK's `ClientSession` parses both, delivering server-initiated notifications to `_handle_notification`, which forwards `tools/list_changed` to the host. The one SDK quirk worth noting: the SDK's `initialize()` already sends the standardized `InitializedNotification` itself, so the gateway does **not** send a second one.

**`tools/list_changed` dispatch (mcp 2.1.1 fix).** Notifications to connected hosts are now **coalesced and sent from a detached task** — never inside a request handler's task/cancel scope. A slow control tool previously held the session open in its own scope while its connect (e.g. `mcp_set_enabled`'s ~9.5 s serial connect) emitted a ~50-message `tools/list_changed` burst; when the burst interleaved into that scope, mcp 2.1.1's dispatcher desynced its cancel-scope stack and crashed the session — every later MCP call died with `Session not found`. Coalescing folds bursts into one send with **trailing-edge re-sends** that keep the freshest catalog (hosts re-list on receipt, so no ordering is assumed); `_active_sessions` is bounded so dead sessions can't widen the fan-out. `mcp_set_enabled` now returns fast ("enabling") with the connect running in the background under its own lock. Applied to `mcp_gateway`, `host_server`, and `job_coding`.

Exposed management tools (LLM-visible as `mcp_set`, `mcp_set_enabled`, `mcp_remove`, `mcp_list`, `mcp_list_tools`), plus the tool-catalog search tool described below (`mcp_tool_search`). Live status is reported by `check_mcp_gateway` via the internal `__check` (which returns `{"servers": [...], "semantic": {...}}` — live server state plus the semantic-index block). The tool-call bridge `__mcp_call_tool` is an internal tool — LLM-invisible, invoked only by the `server__tool` proxies.

`mcp_list` is a static config view — the configured servers (name, transport, command/args or url, enabled/disabled, `auto_load`, description), with no live state and no secrets (env/headers/auth omitted). `check_mcp_gateway` (a standalone tool, also run by `system_health`) calls the internal `__check` for the raw live server state and adds health levels (ok/warning/info) with remediation hints. The separation keeps "what is configured" distinct from "what is connected", so the LLM picks the right tool.
...
`{server}__{tool}` with name, description, the tool's **complete
`tools/list` schema** (the `{name, description, inputSchema}` descriptor as
one compact JSON text in `input_schema`), and a per-tool `enabled` flag
...
> **Terminology.** This store is the **Turns DB**: the memdb
...
> which the memfiles plugin writes. In this document
...
`completed_at` is written for every new turn; fresh databases get the column from `schema.sql`. The `images` column (user image attachments), the `context_tokens` column (context size at the last call), and the `turn_channel` table likewise live in `schema.sql` for fresh databases. There is **no general migration layer** — backward compatibility is not supported (reset the DB) — the one exception being the `prompt_tokens` → `context_tokens` rename, shipped as the standalone `scripts/migrate_context_tokens.py` for existing databases. `restore` still tolerates a missing `context_tokens` value by falling back to the token estimate.

Per-turn token consumption is queryable via **`turn_token_usage`** (`rowid`, `since`/`until`, `limit`) — returns each matching turn's `token_count` (billing) and `context_tokens` (context size) plus totals/averages.
...
Search hardening: every `LIKE` path (`turn_count` grep, `search_like` CJK fallback, `search_grep`, and the memfiles / mcp-gateway search paths) escapes `%`/`_`/`\` through a shared `_like_escape` helper and pairs it with an `ESCAPE '\'` clause; `turn_count` honors `since`/`until` in fts5 mode; LLM-facing search clamps `limit` to `[1, 200]`; the index drainer counts only stored embeddings so it gives up on a persistently failing embedder instead of spinning. Semantic search is gated on index completeness (`SemanticManager.semantic_ready`) — hybrid degrades to FTS5 while any turn lacks an embedding. `turn_search` is a pure read of the gate (no reindex side effect); the drainer converges the index on its own and the gate re-opens when `count_unembedded() == 0`.
...
**Model / dimension change.** The `embeddings_*` native tools persist the top-level `embeddings` section and then hot-reload: they call the internal `__memory_reload_semantic` / `__memfiles_reload_semantic` tools, which `await manager.enable()` — stopping the drainer, migrating the vec0 table in place (`reconfigure_for_embedding` — the schema re-run compares the vec0 `float[N]` width and the current model identity (`backend:model`, persisted in `diary_meta.embedding_model`) against what the DB was built with; a width or model mismatch drops and recreates `diary_semantic`, since old vectors live in a different vector space; the identity does not include the endpoint, so two providers serving the same model name are not distinguished), and restarting the drainer to rebuild. `embeddings_enable(false)` calls `manager.disable()` instead. A failed reload degrades to "takes effect on restart" (never blocks the persist). The same path catches a model/dimension change made by a manual `slife.json5` edit before the gate re-enables.
...
The restored turn prompt is primed with the **latest restored turn's persisted `context_tokens`** — the last call's prompt + completion, i.e. the exact context size at exit (what `_turn_prompt` would have reported) — so the first `_turn_prompt` / status bar shows the real occupancy instead of an estimate. A missing/zero value (e.g. a cancelled turn that never made an API call) falls back to the token estimate. The `context_tokens` column ships in `schema.sql`; older databases are migrated by `scripts/migrate_context_tokens.py`.
...
- Async task results **auto-push by default**: a peer's result arrives on `Slife/<agent_name>/tasks/result` over MQTT → the client fires `on_task_result` → the plugin queues a completion → `_a2a_poll_loop` drains it into the history ("Peer X completed async task (ID: …): …"). The agent sends async and the result simply arrives — no polling or blocking (MQTT subscription is implicit, so the HTTP/SSE-style `a2a_subscribe_task` was dropped). `a2a_send_task_async` takes `mode="auto"` (default — auto-push) or `mode="poll"` (no push; retrieve via `a2a_get_task_result`), mirroring the subagent delivery mode. An auto-pushed result also stays retrievable via the get-result tool, so a caller may poll it in the same turn without missing it.
- **Stateless message tools**: `a2a_send_message` / `a2a_send_message_async` express the A2A standard's conversational message semantics — the same `SendMessage` wire, but **not recorded in the task store** (an unrecorded corr_id makes `record_result`/`record_error` no-ops, so `a2a_list_tasks` / `a2a_cancel_task` never see messages). The plugin tracks outbound message corr_ids → peer in `_message_sends` so an async completion is framed as "Peer X replied to your message" (kind `message`) instead of a task completion; `mode` works exactly as the task async, and the reply still lands in the client's `_completed_tasks` for polling via `a2a_get_task_result`. No `AgentCard.skills` gating — all peers are equal.
- **Channel A2A markers**: the sender's wire envelope carries `_slife.kind` (`"task"` or `"message"`, from `record`), so the receiving side can tell the two apart. Inbound messages arrive at the model prefixed `[A2A:{"agent_name": …, "task_id": …}]` — `task_id` present only for a task (a stateless message carries just the peer), which is how the LLM distinguishes them; auto-pushed async results are prefixed `[A2A-PUSH:…]` with the same shape. Both markers are machine-facing: the TUI shows `A2A(<peer>)>` and `unwrap_info_envelope` strips them (like the Wechat/Subagent markers).
...
- **Async delivery mode**: `subagent_send_task_async` takes `mode="auto"` (default — the result is auto-pushed to the parent's inbox when the worker completes, starting a new turn; it **also stays retrievable** via `subagent_get_task_result`, so a caller may poll the same result in-turn) or `mode="poll"` (no push — retrieve via `subagent_get_task_result`). The mode is chosen at send time so the caller's intent is explicit: `poll` suppresses the push so the result is integrated without a surprise new turn, `auto` guarantees delivery even if the caller never polls. Both modes keep the result in the get-result store, so polling a pushed result is always safe.
...
`url_save` guards against SSRF before fetching — and re-runs the guard on **every** redirect hop: every resolved address must be globally routable, so loopback / private / link-local / cloud-metadata (`169.254.169.254`) targets are refused. One deliberate exception: the Clash/sing-box **fake-ip range** `198.18.0.0/15` is accepted, because those resolvers answer real public hostnames with addresses from it — and Python's `ipaddress` flags the range as private, which would otherwise refuse *every* URL on a fake-ip machine.

Each kind owns its FTS5 + vec0 tables (`notes_*` / `diary_*` / `files_*`). `cabinet_search(query, kind, mode)` runs hybrid (FTS5 + vec0 KNN, RRF via the shared `merge_hybrid`) or keyword search across them; `cabinet_read(path)` re-opens a file with a path-traversal guard. Browsing by key: `note_list` / `diary_list` list entries (newest first, diary optionally by date range) and `note_read(subject)` / `diary_read(date)` return full content. The index mirrors memdb's design and **reuses its code**: the shared `SemanticManager` (document-source contract) drives the drainer over all three kinds, and each plugin's internal `__check` reports its own gate — memdb and memfiles independent, because each plugin reindexes its own DB (one shared top-level `embeddings` config, independent availability) — aggregated into `system_health`.
...
**Trigger → execution.** The loop injects a `[Schedule <name>]` message into the unified inbox under the **system channel** (source `SYSTEM`, `channel=system`, silent handler like heartbeat — excluded from "remote" handling in `inbox.py`); the `[Schedule <name>]` text is the *schedule marker* riding that channel's content. The run itself is recorded when the agent dispatches, not at fire time, and an in-memory pending-fire guard keeps the 30 s poll from re-firing while the agent is mid-turn — the guard is dropped only on age **and** after checking the trigger's `[Schedule <name>]` message is no longer still queued in the inbox, so a long human/remote turn that outlasts the grace window can never double-fire the same task. The agent handles the trigger by delegating, not doing: it calls `run_schedule_now` — the single dispatch tool, also used to backfill missed runs — which records a `scheduled_runs` row (`status='pending'` — success is unconfirmed until a report lands), spawns/reuses the subagent named after the task, and sends it a deterministic task text via `subagent_send_task_async` (mode=auto) that instructs the worker to call `report_save` when done and to notify the user. Completion rides the existing subagent auto-push back to the main agent, which reports the task as finished (reworded to hide the subagent). Idle schedule workers are recycled by the loop once their task has no run still in flight.

**Record — three memfiles tables.** `scheduled_tasks` (definition: name, description, cron, timezone, enabled — stored in the DB so tasks are runtime-mutable, added in conversation), `scheduled_runs` (per-fire state + report link), and `reports` (a fourth cabinet document kind, `reports/<slug>.md`, with FTS5 + vec0 indexes via the `_KIND_SPECS` extension). `report_save` writes the report and — when bound to a task — backfills the newest un-linked run's `report_id` at the store layer, so the run→report link closes without depending on a second LLM call. A `report_save` with no task is a standalone report (`reports.task_id` NULL on fresh DBs; older databases keep NOT NULL and reject it with a clear error). Fresh DBs get the tables from `schema.sql`; the runtime self-heals `reports_semantic`. Older databases are out of scope (backward compatibility is not supported).
...
Tools: `scheduled_task_set` / `scheduled_task_remove` / `scheduled_task_list`, `scheduled_run_list` / `scheduled_run_skip`, and `run_schedule_now` — all native, in the "Schedule" category (`slife/tools/schedule.py`, visible to main agent and worker alike; the run/task data they drive lives in the memfiles plugin's `scheduled_tasks` / `scheduled_runs` tables, reached via its internal `__scheduled_*` tools). `run_schedule_now` is the single dispatch tool for both cron fires and backfills — it records the run and dispatches the worker directly, reached through the `fire_schedule_now` tool-context hook. Report tools `report_save` / `report_list` / `report_read` stay memfiles MCP tools (reports are a cabinet document kind). `scheduled_task_set` validates that a task name is a safe identifier, because it doubles as the worker name. `run_schedule_now` takes `due_at` (backfill a missed fire) and `clone_context=True` (spawn the worker with a clone of the main agent's current conversation — for a task with no description or one that depends on what was just discussed; degrades to a clean spawn when no history is reachable). `scheduled_task_set`'s `description` is **schema-required** — it is the worker's instruction, so a task with no description cannot exist.
...
Health checks fall into two categories. `system_health` runs all of them together, and every dynamic check is also exposed as a standalone native tool (`check_memdb`, `check_wechat`, `check_memfiles`, `check_local_embed`, `check_sharefile`, `check_mcp_gateway`, `check_a2a`, `check_watchdog`) so the LLM can probe a single subsystem directly without the full report. `check_mcp_gateway` additionally takes an optional `server` argument (default: all) to diagnose just one external server.
...
slife/plugins/mcp_gateway/      # The MCP gateway — a built-in plugin (no standalone package)

...
Exposed management tools: `mcp_set`, `mcp_set_enabled`, `mcp_remove`, `mcp_list`, `mcp_list_tools` (LLM-visible); `__check`, `__mcp_list_tools`, `__mcp_call_tool` (internal — the `__` prefix keeps them out of the model's tool set).  `mcp_connect` / `mcp_disconnect` were retired with the modern protocol era — a stateless peer has no session to open or close, so `mcp_set_enabled` is the single lifecycle switch (enabling reads; a later tool call re-establishes lazily). `mcp_list` is a **static config view** — what is configured, with no live state and no secrets; the internal `check_mcp_gateway` function (run by `system_health`, not callable as a tool) reads the same `__check` for the raw live state and adds health levels with remediation hints. The separation keeps "what is configured" distinct from "what is live".

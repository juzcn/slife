...
```yaml
net:
  fake_ip_exempt: auto    # off（默认）| auto | on
```

| 取值 | 效果 |
|------|------|
| `off` | 非公网 IP 字面量一律拒绝。默认值——而且是有意为之（见下面的代价）。 |
| `auto` | 仅当实测到本机 resolver 在说谎时豁免 fake-IP 池，也就是池中地址确实属于代理的合成空间时。 |
| `on` | 无条件豁免，用于探针看不见的代理——`fake-ip-filter` 列出了探针域名，或探针在代理启动前就跑过了。 |

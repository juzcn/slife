...
空闲时，agent 按 `agent.heartbeat_interval` 秒（默认 1800；设为 0 关闭心跳）获得一个周期性的自主窗口。它作为一个正常 turn 运行（独立的一轮，存入记忆）；回复契约是：有值得说的话就输出真实内容，否则只输出一个 `.`。单独的 `.` 回复表示**沉默**——来自任何事件（心跳、A2A 异步完成通知等）的 `.` 都不会渲染到聊天或会话恢复里；`[Heartbeat]` 触发消息被过滤，真正的自主回复显示为 `⚡ 自主`。这是涌现自发性行为的前提。
...
| 插件 | 角色 |
|--------|------|
| **mcp-gateway** | MCP 网关——代理外部 MCP 服务器（stdio / SSE / Streamable HTTP）并持有这些连接。管理：`mcp_set`、`mcp_set_enabled`、`mcp_remove`、`mcp_list`、`mcp_list_tools`（有截断——其余用 `tool_search` 找）；工具经 `func_tool_load` 按需载入 |
| **memdb** | 对话记录数据库 + 混合搜索 |
| **wechat** | 双向微信消息 |

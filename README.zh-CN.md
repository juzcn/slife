...
空闲时，agent 按 `agent.heartbeat_interval` 秒（默认 60）获得一次自主思考/行动的窗口。它作为一个正常 turn 运行（独立会话，存入记忆）；回复契约：有值得说的话就输出内容，否则只输出一个 `.`——`. ` 和 `[Heartbeat]` 触发消息会被 TUI 过滤，真正的自主回复显示为 `⚡ 自主`。这是涌现自发性行为的前提。

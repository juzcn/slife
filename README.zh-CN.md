...
- **已存在的配置**保持原样；只有在其内容与随包默认值**不同**时，才**逐个文件**询问是否重置为默认（完全相同的文件静默跳过；默认：否）。缺失的配置静默铺设。
- **已存在的 skills** 保持原样；只有在其内容与随包默认值**不同**时，才**逐个 skill** 询问是否覆盖（完全相同的文件静默跳过；默认：否）。缺失的 skills 静默铺设。

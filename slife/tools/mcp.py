...
(``__mcp_gateway_get_tool``) and registers an :class:`~slife.mcp.tool_adapter.MCPProxyTool`,
...
server granularity (``mcp_gateway_remove`` / ``mcp_gateway_set_enabled``) or when a tool is
...
class McpToolLoadTool(Tool):
    ...
    description = (
        ...
        "LLM's tool list (find it with mcp_gateway_tool_search)."
    )
    parameters = {
        "type": "object",
        "properties": {
    ...
    async def execute(self, **kwargs) -> str:
        ...
        try:
            raw = await mcp.call_tool("__mcp_gateway_get_tool", {"full_name": full_name})
            data = json.loads(raw)
        ...
        if not data.get("enabled", True):
            return (
                ...
                f"disabled). Enable it with mcp_gateway_set_enabled, then load again."
            )

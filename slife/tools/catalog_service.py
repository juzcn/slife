...
class ToolCatalogService:
    ...
    async def load_tool(self, name: str) -> tuple[bool, str]:
        ...
        The success message states NO timing, and that is the decision.  What a
        load changes is what a turn INJECTS: the injection list for the request
        already in flight was built before this flip, so the schema lands in
        the next one.  But injection is not what makes a call possible — the
        registry runs any name that has an instance behind it and never gates
        on ``load_status`` (:func:`slife.tools.registry.not_loaded_refusal`) —
        so a load and a well-formed call to the same tool in one message both
        succeed.  Stating the timing reads as the opposite ("wait for the next
        request"), which is exactly how a model ends its turn instead of
        calling the thing it just loaded; the sibling call is the fact it
        needs, and this wording leaves it to do that.
        """

"""Model management — list, add, remove, and switch LLM models.

list_models:     list all configured models grouped by provider
add_model:       add a model to a provider (create provider if new)
remove_model:    remove a model from a provider
switch_model:    switch the active model

Models are persisted to slife.json5 → models.providers: section and
active_model: key.  These tools follow the same pattern as CLI tools
